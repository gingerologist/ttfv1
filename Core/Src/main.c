...
void DDS_Start(uint32_t freq, bool print) {
  ...
  if (print) {
    printf("dds freq: %lu, lo: 0x%04x, hi: 0x%04x\r\n", freq, freq_lo, freq_hi);
  }
...
static void MX_GPIO_Init(void) {
  ...
  HAL_GPIO_WritePin(GPIOB, LEDC_Pin | MUX_CLR_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : SW4_Pin SW5_Pin PC10 PC11
                           PC12 */
  GPIO_InitStruct.Pin =
      SW4_Pin | SW5_Pin | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

  /*Configure GPIO pins : LEDC_Pin MUX_LE_N_Pin MUX_CLR_Pin */
  GPIO_InitStruct.Pin = LEDC_Pin | MUX_LE_N_Pin | MUX_CLR_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
