...
HAL_StatusTypeDef TCA9555_WriteReg(uint8_t dev_idx, uint8_t reg, uint8_t value)
{
    ...
    return HAL_I2C_Master_Transmit(&hi2c1, TCA9555_DEVICES[dev_idx], data, 2, HAL_MAX_DELAY);
}
...
HAL_StatusTypeDef TCA9555_ReadReg(uint8_t dev_idx, uint8_t reg, uint8_t *value)
{
    ...
    if (HAL_I2C_Master_Transmit(&hi2c1, TCA9555_DEVICES[dev_idx], &reg, 1, HAL_MAX_DELAY) != HAL_OK) {
        return HAL_ERROR;
    }

    // Read data
    return HAL_I2C_Master_Receive(&hi2c1, TCA9555_DEVICES[dev_idx], value, 1, HAL_MAX_DELAY);
}
...
HAL_StatusTypeDef TCA9555_WriteGPIO(uint8_t dev_idx, uint16_t gpio_state)
{
    ...
    status = TCA9555_WriteReg(dev_idx, TCA9555_REG_OUTPUT_PORT0, (uint8_t)(gpio_state & 0xFF));
    if (status != HAL_OK) {
        return status;
    }

    // Write P1 pins (upper byte)
    status = TCA9555_WriteReg(dev_idx, TCA9555_REG_OUTPUT_PORT1, (uint8_t)((gpio_state >> 8) & 0xFF));
